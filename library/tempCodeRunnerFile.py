...
def insert_customer():
    ...
    db=connection()
    cursor=db.cursor()
    cursor.execute("insert into customers values(?,?,?,?,?)",(id,name,address,Mobile_No,join))
    db.commit()
    db.close()
...
def update():
    ...
    db=connection()
    cursor=db.cursor()
    cursor.execute("update lending set return_date=? where username=? and b_id=?" ,(returned_date,username,b_id))
    cursor.execute("update lending set returned=? where username=? and b_id=?",(returned,username,b_id))
    db.commit()
    db.close()
    return render_template("books.html")
